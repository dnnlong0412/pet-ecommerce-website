...
router.get('/tim-kiem.html', async (req, res) => {
  ...
  const query = {
    isDeleted: false,
    // Thanh tìm kiếm "TÌM KIẾM THÚ CƯNG" chỉ trả về chó/mèo
    categoryId: { $in: [8, 9] }
  };

  const rawKeyword = (req.query.keyword || '').trim();
  model.keyword = rawKeyword;
  ...
  if (rawKeyword.length > 0) {
    ...
    // Nếu search đúng "chó" / "mèo" thì trả về toàn bộ giống chó/mèo
    if (normalizedCompact === 'cho' || normalizedCompact === 'cho canh') {
      query.categoryId = 8;
    } else if (normalizedCompact === 'meo' || normalizedCompact === 'meo canh') {
      query.categoryId = 9;
    } else {
      // Search theo tên giống, chỉ trong chó/mèo
      query.name = RegExp(escaped, 'i');
    }
  }
  else {
    // Không nhập gì thì không trả về kết quả
    model.products = [];
    model.categories = await CategoryModel.find({ isDeleted: false }).lean();
    return res.render('site/tim-kiem', model);
  }
